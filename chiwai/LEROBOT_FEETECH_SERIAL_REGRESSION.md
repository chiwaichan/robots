...
2. **Gripper stalls open under power — it is TORQUE-limited, not hard-stopped.**
   ...
   - **Root cause (VERIFIED 2026-07-01):** a **standing-error stall → I²R heat**, not a fault. Commanded to its open extreme, the gripper can't reach the goal under the (deliberately) capped torque, so it stalls holding a position error; a blocked rotor does no work → all drive power becomes winding heat. **The conservative limits are an intentional anti-burnout mitigation** — [lerobot PR #1809](https://github.com/huggingface/lerobot/pull/1809) added `Max_Torque_Limit=500`/`Overload_Torque=25`/`Protection_Current=250` specifically to stop gripper burnout "reported by Seeed Studio." Idle measurement confirmed the servo is **healthy** (held at a reachable position it drew **0 mA and cooled** 50→42°C). Leaving torque ON when parked (`disable_torque_on_disconnect=False`) made the heat persist across runs.
   - **Proper fix — do NOT raise the torque** (that re-introduces the burnout PR #1809 prevents): make the open goal reachable. Recalibrate `range_max` a few degrees *inside* the open stop, cap the open command (`test_servo_range.sh --gripper-max 74`), and release gripper torque when idle. Full write-up: [`SO101_SERVO_RANGES.md`](./SO101_SERVO_RANGES.md) → *Finding 2 → VERIFIED root cause*.
